...
function NationalCarrierMap({
  ...
  const annualTarget = 300000000;
  const monthlyTarget = 25000000;
  const dailyTarget = 833333;
  const hourlyTarget = annualTarget / 365 / 24;
     ...
     L 360 45 Z"
      ...
      <g transform="translate(688 104)">
        ...
        {[
          ...
          [
            'التدفق التصميمي',
            `${hourlyTarget.toLocaleString(undefined, {
              maximumFractionDigits: 0,
            })} m³/h`,
            '#86efac',
          ],
  ...
  return (
    ...
    >
      <defs>
        ...
        <linearGradient id="pipeGradBurst" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#fee2e2" />
          <stop offset="24%" stopColor="#fca5a5" />
          <stop offset="58%" stopColor="#ef4444" />
          <stop offset="100%" stopColor="#991b1b" />
        </linearGradient>
        <linearGradient id="pipeGradLeak" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#fffbeb" />
          <stop offset="24%" stopColor="#fde68a" />
          <stop offset="58%" stopColor="#f59e0b" />
          <stop offset="100%" stopColor="#92400e" />
        </linearGradient>
        <linearGradient id="pipeGradTheft" x1="0" y1="0" x2="0" y2="1">
...
export default function AqurdDashboard() {
  ...
  // 15-second SYNCHRONIZED PUMP CYCLE
  useEffect(() => {
    ...
    >
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            >
              ...
              >
                {worstType === 'normal' && 'النظام مستقر — لا توجد حالة طارئة'}
                {worstType === 'leak' && 'تم اكتشاف تسرّب في الشبكة'}
                {worstType === 'burst' && 'تم رصد انفجار حرج في الشبكة'}
                {worstType === 'theft' && 'تم رصد اشتباه سرقة مياه'}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {worstType === 'normal' &&
                  [
                    'جميع القراءات ضمن الحدود التشغيلية الطبيعية.',
                    'لا يتطلب الوضع أي تدخل ميداني في الوقت الحالي.',
                  ].map((s, i) => (
                    <div
                      key={i}
                      style={{
                ...
                {worstType === 'leak' &&
                  [
                    'توجيه فريق الكشف الميداني باستخدام وسائل التحسس الصوتي.',
                    'خفض ضغط الشبكة بنسبة 15٪ مؤقتاً للحد من الفاقد.',
                    'فتح أمر صيانة ومعالجة العطل بأولوية متوسطة.',
                  ].map((s, i) => (
                    <div
                      key={i}
                      style={{
                ...
                {worstType === 'burst' &&
                  [
                    'عزل المقطع المتضرر فوراً لمنع تفاقم الخسائر.',
                    'إشعار فريق الطوارئ والتحرك الميداني بشكل عاجل.',
                    'إرسال طاقم الصيانة بزمن استجابة لا يتجاوز ساعتين.',
                    'توثيق كميات الفاقد ورفع تقرير تشغيلي للإدارة.',
                  ].map((s, i) => (
                    <div
                      key={i}
                      style={{
                ...
                {worstType === 'theft' &&
                  [
                    'فحص التوصيلات الجانبية والاستهلاكات غير النظامية ميدانياً.',
                    'مراجعة سجلات الاستهلاك لآخر 72 ساعة.',
                    'التنسيق مع وحدة حماية الإيرادات لاتخاذ الإجراء اللازم.',
                  ].map((s, i) => (
                    <div
                      key={i}
                      style={{
      ...
      <style>{`
        ...
        input[type=range]::-webkit-slider-thumb:hover{
          ...
          box-shadow:0 0 0 5px rgba(99,102,241,.24);
        }
